...
<div class="flex-grow flex flex-col gap-2 py-2">
	...
	<div class="relative text-slate-400 isolate grid grid-cols-2 items-center w-full bg-neutral-700">
		<span
			class="absolute -z-10 left-0 w-1/2 h-full bg-neutral-800/50 {uiMode
				? ''
				: 'translate-x-full'} transition-transform"
		/>
		<button class="p-2 {uiMode ? '' : 'hover:bg-neutral-600'}" on:click={() => (uiMode = true)}
			>UI</button
		>
		<button class="p-2 {uiMode ? 'hover:bg-neutral-600' : ''}" on:click={() => (uiMode = false)}
			>CODE</button
		>
	</div>
</div>
